...
class InterfaceApp:
    def __init__(self, master=None):
        ...
        self.algorithmbox.configure(cursor="hand2", state="readonly",
                                    values=('Breadth-First', 'Depth-First', 'A Star (A*)', 'Uniform-Cost', 'Depth-Limited', 'Iterative Deepening', 'Greedy'))
        ...
        self.gif = [tk.PhotoImage(file='assets/loading.gif', format='gif -index %i' % i) for i in range(10)]
    ...
    def solveState():
        ...
        if str(algorithm) == 'Breadth-First':
        ...
        elif str(algorithm) == 'Depth-First':
            main.DFS(initialState)
            path, cost, counter, depth, runtime = \
                main.dfs_path, main.dfs_cost, main.dfs_counter, main.dfs_depth, main.time_dfs
        elif str(algorithm) == 'A Star (A*)':
            main.AStarSearch(initialState)
            path, cost, counter, depth, runtime = \
                main.astar_path, main.astar_cost, main.astar_counter, main.astar_depth, main.time_astar
        elif str(algorithm) == 'Uniform-Cost':
            main.uniform_cost_search(initialState)
            path, cost, counter, depth, runtime = \
                main.ucs_path, main.ucs_cost, main.ucs_counter, round(main.ucs_depth), main.time_ucs
        elif str(algorithm) == 'Depth-Limited':
            main.depth_limited_search(initialState)
            path, cost, counter, depth, runtime = \
                main.dls_path, main.dls_cost, main.dls_counter, round(main.dls_depth), main.time_dls
        elif str(algorithm) == 'Iterative Deepening':
            main.iterative_deepening_search(initialState)
            path, cost, counter, depth, runtime = \
                main.ids_path, main.ids_cost, main.ids_counter, round(main.ids_depth), main.time_ids
        elif str(algorithm) == 'Greedy':
            main.greedy_search(initialState)
            path, cost, counter, depth, runtime = \
                main.greedy_path, main.greedy_cost, main.greedy_counter, round(main.greedy_depth), main.time_greedy
